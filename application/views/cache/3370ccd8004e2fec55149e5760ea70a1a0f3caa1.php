<?php /* /var/www/codeigniter/application/views/master/users/edit.blade.php */ ?>
<?php
/**
 * Created by PhpStorm.
 * User: hana
				...
				<div class="form-group">
					...
					<select id="role" class="form-control" name="txtRole">
						<?php $__currentLoopData = $role; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							<?php if($item->id != 1): ?>
								<option value="<?= $item->id ?>" <?php if($user->id_per == $item->id): ?> selected <?php endif; ?>><?= $item->name ?></option>
							<?php endif; ?>
