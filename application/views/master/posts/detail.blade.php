...
@section('content')
            ...
            <div id="accordion">
                ...
                <div class="accordion">
                    ...
                    <div class="accordion-body collapse" id="panel-body-3" data-parent="#accordion" style="">
                        @if(in_array('0',$arr->comments) || in_array('1',$arr->comments))
                            @foreach($cm as $k => $comment)
                                <div class="user-cm mt-1 com<?= $comment->id_com ?>">
                                <span class="comment<?= $comment->id_com ?>"
                                      @if($comment->status == 2) style="opacity: 0.4" @endif>
                                        @if($comment->id_google != 0)
                                ...
                                <a href="#"><?= $comment->name ?></a>: <?= $comment->content ?>
                                    ...
                                    @if(in_array('0',$arr->comments) || in_array('3',$arr->comments) || in_array('4',$arr->comments))
                                        ...
                                        <a href="#"><?= $reply->name ?></a>: <?= $reply->content ?>
                                            ...
                                            <div class="dropdown-menu" x-placement="bottom-start"
                                                ...
                                                <a class="dropdown-item has-icon hide hidden<?= $reply->id_com ?>"
                                                   ...
                                                   @if($reply->status == 2) style="display: none;" @endif><i
                                                            class="fas fa-exclamation-triangle"></i>Ẩn
                                                    bình luận</a>
                                                <a class="dropdown-item has-icon show-block show<?= $reply->id_com ?>"
                                                   href="#" data-id="<?= $reply->id_com ?>"
                                                   @if($reply->status == 1) style="display: none;" @endif><i
                                                            class="fas fa-exclamation-triangle"></i>Hiển thị
                                                    bình luận</a>
                                                @endif
                                                    @if(in_array('0',$arr->comments) || in_array('4',$arr->comments))
    ...
    <div class="footer-detail float-right">
        @if(in_array('0',$arr->news) || in_array('3',$arr->news))
            @if($new->status != 1)
            ...
            @endif
        @endif
        @if(in_array('0',$arr->news) || in_array('4',$arr->news))
